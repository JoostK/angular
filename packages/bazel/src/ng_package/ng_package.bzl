...
def _ng_package_impl(ctx):
    ...
    for dep in ctx.attr.deps:
        ...
        entry_point = dep.label.package[len(owning_package) + 1:]
        ...
        if hasattr(dep, "angular") and hasattr(dep.angular, "flat_module_metadata"):
            ...
            es2020_entry_point = _find_matching_file(unscoped_esm2020, "%s/index.mjs" % owning_package)
            typings_file = _find_matching_file(unscoped_type_definitions, "%s/index.d.ts" % owning_package)
            metadata_file = None
            guessed_paths = True
