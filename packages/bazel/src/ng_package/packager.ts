...
function main(args: string[]): void {
  ...
  Object.keys(metadata.entryPoints).forEach(moduleName => {
    const entryPointSubpath = getEntryPointSubpath(moduleName);
    const isSecondaryEntryPoint = entryPointSubpath !== '';

    if (isSecondaryEntryPoint && !packagesWithExistingPackageJson.has(moduleName)) {
      createSecondaryEntryPointPackageJson(entryPointSubpath, moduleName);
    }
  });
  ...
  function updatePrimaryPackageJson(packageJson: Readonly<PackageJson>): PackageJson {
    ...
    if (packageJson.exports !== undefined) {
      throw Error(
          'The primary "package.json" file of the package sets the "exports" field ' +
          'that is controlled by the packager. Please unset it.');
    }
    ...
    // TODO: Setup type definitions here once TypeScript supports it and consider dropping
    //  secondary package.json files: https://github.com/microsoft/TypeScript/issues/33079.
    newPackageJson.exports = {
      '.': {default: newPackageJson['fesm2020']},
    ...
    // Capture all secondary entry-points in the `exports` field using the subpath
    // export declarations: https://nodejs.org/api/packages.html#packages_subpath_exports.
    for (const [moduleName, entryPoint] of Object.entries(metadata.entryPoints)) {
      const subpath = getEntryPointSubpath(moduleName);
      const fesmOutRelativePath = getFlatEsmOutputRelativePath(entryPoint.fesm2020Bundle);

      newPackageJson.exports[`./${subpath}`] = {default: normalizePath(fesmOutRelativePath)};
    }
